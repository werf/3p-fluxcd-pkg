...
// getClusterConfigMapGCP returns the cluster configmap for kubeconfig auth tests.
func getClusterConfigMapGCP(output map[string]*tfjson.StateOutput) (map[string]string, error) {
	clusterResource := output["cluster_resource"].Value.(string)
	if clusterResource == "" {
		return nil, fmt.Errorf("no GKE cluster id in terraform output")
	}
	clusterAddress := output["cluster_endpoint"].Value.(string)
	if clusterAddress == "" {
		return nil, fmt.Errorf("no GKE cluster address in terraform output")
	}
	return map[string]string{
		meta.KubeConfigKeyProvider: gcp.ProviderName,
